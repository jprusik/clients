...
export default class NotificationBackground {
  ...
  async triggerChangedPasswordNotification(
    ...
    // An unlocked vault is needed to determine which kind of notification should be
    // sent (if any), but if no cipher add/update notifications are enabled,
    // we can exit early.
    ...
    // If there is no account logged in (as opposed only being locked), exit early
    const authStatus = await this.getAuthStatus();
    if (authStatus === AuthenticationStatus.LoggedOut) {
      return false;
    }
  ...
  private async handleInputMatchScenario({
    ...
    // Note, this block may uniquely exit early since this match scenario involves all ciphers
    if (noFieldMatches.length === ciphersForURL.length) {
      ...
      // trigger an update cipher notification will all URI ciphers in these input scenarios
      if (
