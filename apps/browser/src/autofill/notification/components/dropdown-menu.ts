...
let showDropdown = false;
  const handleButtonClick = (event: Event) => {
    ...
    if (!disabled) {
      // show dropdown
      console.log('dropdownIsOpen before:', showDropdown);
      showDropdown = !showDropdown;
      console.log('dropdownIsOpen after:', showDropdown);
  ...
  return html`
    <div class=${dropdownContainerStyles}>
      ...
      ${showDropdown ? html`
        <div class=${dropdownMenuStyles({theme})}>
          Coooool
        </div>
      ` : null}
    </div>
...
const dropdownButtonStyles = ({ disabled, theme }: { disabled: boolean; theme: Theme }) => css`
  ...
  ${disabled
    ...
    `
    : `
      border: 0.5px solid ${themes[theme].text.muted};
