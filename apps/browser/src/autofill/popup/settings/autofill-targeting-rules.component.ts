...
export class AutofillTargetingRulesComponent implements AfterViewInit, OnDestroy {
  ...
  constructor(
    ...
    // private autofillSettingsService: AutofillSettingsService,
  ...
  async ngAfterViewInit() {
    this.domainSettingsService.autofillTargetingRules$
      ...
      .subscribe((targetingRulesSet: AutofillTargetingRulesByDomain) => this.handleStateUpdate(targetingRulesSet));
  ...
  handleStateUpdate(targetingRulesSet: AutofillTargetingRulesByDomain) {
    if (targetingRulesSet) {
      this.storedBlockedDomains = Object.keys(targetingRulesSet);
    }
  ...
  async saveChanges() {
    ...
    const newBlockedDomainsSaveState: AutofillTargetingRulesByDomain = {};
    ...
    for (const uri of uniqueBlockedDomains) {
      if (uri && uri !== "") {
        ...
        newBlockedDomainsSaveState[validatedHost] = {};
      }
    }
    ...
    try {
      ...
      if (stateIsUnchanged) {
        ...
        const constructedNeverDomainsState = this.storedBlockedDomains.reduce(
          (targetingRulesSet: AutofillTargetingRulesByDomain, uri: string) => ({ ...targetingRulesSet, [uri]: {} }),
          {},
        );
        this.handleStateUpdate(constructedNeverDomainsState);
      } else {
        await this.domainSettingsService.setAutofillTargetingRules(newBlockedDomainsSaveState);
      }
